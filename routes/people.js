...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        req.flash('success', 'Adiciondo com sucesso.');
    } catch (error) {
        console.error(erro);
        req.flash('error', 'error ao adicionar pessoa.');
        throw erro;
    }
    res.redirect('/people');
});
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        await db.execute('DELETE FROM person WHERE id=?', [IDP]);
        req.flash('success', 'Excluido com sucesso.');
    } catch (error) {
        console.error(erro);
        req.flash('error', 'error ao excluir pessoa.');
        throw erro;
    }
    res.redirect('/people');
});
